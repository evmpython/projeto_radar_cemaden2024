...
def _get_data(rawdata, nrays, nbins):
    ...
    # fill nbins data out the range with fillvalue
    if data.shape[1] < nbins:
        #print(data.shape,nrays, nbins)
        mask_out = np.full((nrays, nbins), get_fillvalue())
        mask_out[:, 0:data.shape[1]] = data
        data = mask_out
        mask = data == get_fillvalue()
